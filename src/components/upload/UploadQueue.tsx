...
export function UploadQueue({ items, onRetry, onRemove }: UploadQueueProps) {
  ...
  const progressPercent =
    items.length > 0 ? (completed / items.length) * 100 : 0;
...
function UploadQueueItem({ item, onRetry, onRemove }: UploadQueueItemProps) {
  ...
  return (
    <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
      <StatusIcon
        className={`w-5 h-5 shrink-0 ${iconColorClass} ${isAnimated ? "animate-spin" : ""}`}
      />
      ...
      <span
        className={`px-2 py-1 text-xs font-medium rounded-full ${config.badgeClass}`}
      >
        {config.label}
      </span>
